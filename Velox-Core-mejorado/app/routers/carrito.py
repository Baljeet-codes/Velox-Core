...
def obtener_o_crear_carrito(usuario_id: int, db: Session) -> Carrito:
...
def obtener_carrito(
    ...
    return obtener_o_crear_carrito(usuario_id, db)
...
def agregar_item(
    ...
    nueva_cantidad = (item_existente.cantidad if item_existente else 0) + item.cantidad
    ...
    if item_existente:
        ...
        db.add(CarritoItem(carrito_id=carrito.id, producto_id=item.producto_id, cantidad=item.cantidad))
